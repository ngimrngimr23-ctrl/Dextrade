...
from pricing import get_sticker_prices, ingest_manual_prices
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Отличаем два разных типа файла по форме JSON:
    # - market/listings/.../render/ (обычный скан лотов) -> есть "results_html"
    # - market/search/render/ по категории стикеров (ручной прайс-лист) ->
    #   есть "results" списком объектов с "sell_price" у каждого
    results_list = data.get("results")
    if isinstance(results_list, list) and results_list and "sell_price" in results_list[0]:
        items = {}
        for r in results_list:
            name = r.get("hash_name") or r.get("name")
            price_cents = r.get("sell_price")
            if name and price_cents is not None:
                items[html_module.unescape(name)] = price_cents / 100.0
        ...
        await update.message.reply_text(
            ...
            f"Если есть ещё страницы — пришли их так же (start={start + len(results_list)})."
        )
        return

...
# --- Рейт-лимит Steam ---------------------------------------------------
# Изначально стояло 4 сек (по опыту сообщества) — но на практике для этого
# IP (датацентр Render) этого оказалось мало: в логах 2026-08-14 18:12 видно,
# как ровно 4-й запрос подряд, разнесённый по эти же 4 сек, словил 429 —
# т.е. реальный порог для этого IP строже расчётного. Подняли с запасом.
...
# закапывает себя глубже. Вместо ретраев: сразу сдаёмся и ставим глобальный
# кулдаун, в течение которого к Steam не ходим вообще.

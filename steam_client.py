...
#
# Общее правило — «бан у Steam по IP, а не по эндпоинту». Но два эндпоинта из
# него выпадают, и оба по одной причине: они режутся несопоставимо жёстче
# /render/, поэтому их 429 ничего не говорит о доступности листингов.
#
#   inventory — 429 прилетает даже на первом запросе за час с датацентрового
#               адреса.
#   pricing   — ровно то, ради чего области вообще разделили: 2026-08-14/15
#               priceoverview ловил 429 сразу, а /render/ в те же часы не
#               подводил ни разу. 2026-08-27 это повторилось буквально в одном
#               логе: все восемь проверок цены получили 429, а вотчлист в тот
#               же час спокойно собирал листинги с HTTP 200.
#
# Без исключения смысл разделения областей терялся: 429 на некритичной проверке
# цены останавливал бы на полчаса критичный сбор листингов — то есть ровно то,
# от чего разделение и спасало.
#
# Обратное направление сохраняется: 429 на листингах придержит и цены, и
# инвентарь — там посылка про общий бан по IP остаётся в силе.
COLLATERAL_EXEMPT_SCOPES = ("inventory", "pricing")
...
async def fetch_all_listings(
    ...
    # Был ли в этом вызове НАСТОЯЩИЙ отказ Steam. Нужен для честного текста в
    # конце: если на прокси мы ушли из-за 429, винить в провале одни прокси —
    # значит скрыть первопричину. Именно на этом человек и споткнулся: бот
    # писал «Steam тут ни при чём» через четыре секунды после его 429.
    steam_refused = steam_cooldown_remaining("listings") > 0
    start = 0
    total_count = None
    while total_count is None or start < total_count:
        ...
        try:
            ...
            # Кончился БЮДЖЕТ ПОПЫТОК на этом предмете, а свободные адреса
            # остались — прогон останавливать не за что.
            #
            # Бюджет ограничен STEAM_RETRY_HARD_CAP (10), а логинов бывает под
            # сотню. На проде 2026-09-14 это выглядело так: «Пул: 85 свободных
            # из 96» и прогон оборван — то есть попробовали одиннадцать адресов
            # из девяноста шести и сдались за всех. Ровно эту развилку мы уже
            # развели в ветке 429; здесь, в ветке отказа ПРОКСИ, её забыли.
            #
            # Исключение намеренно не SteamRateLimited: оно останавливает весь
            # прогон (см. bot._watchlist_scan_item), а нам нужно уронить только
            # этот предмет. Следующий начнёт со своим свежим бюджетом.
            free_now = STEAM_POOL.available()
            if free_now:
                raise RuntimeError(
                    f"Прокси-сервис отказал на всех {listings_retry_budget()} "
                    f"маршрутах, отведённых этому предмету ({scrub(str(e))}). "
                    f"Свободных логинов ещё {len(free_now)} — прогон не останавливаю, "
                    f"следующий предмет пойдёт как обычно."
                )

            # Сюда попадаем, когда запрос не пропустил ПРОКСИ-СЕРВИС.
            #
            # Кулдаун на область Steam здесь НЕ ставится, и это принципиально.
            # Ставить его — значит замкнуть круг, что и происходило:
            #
            #   1. прокси отдают 403 -> ставим кулдаун на listings
            #   2. следующий прогон видит кулдаун и по правилу «прямой адрес
            #      забанен» СРАЗУ уходит на прокси, не пробуя прямой вовсе
            #   3. прокси снова 403 -> снова кулдаун -> на пункт 1
            #
            # Прямой адрес при этом мог быть давно свободен: бан ему записали
            # МЫ САМИ по чужой вине. Человек ждал часами, а бот честно ни разу
            # не проверял, прошло ли.
            #
            # Теперь состояние Steam не трогаем. Прогон останавливается (идти
            # больше некуда), но следующий начнётся с ПРЯМОГО адреса. Если он
            # и правда забанен — потратим на нём один запрос и снова уйдём на
            # прокси; один запрос раз в интервал автоскана бан не продлевает,
            # а вот бесконечный круг не давал работать вообще.
            # Порядок событий важнее ярлыка «виноваты прокси». Если 429 от
            # Steam был, он и есть первопричина: на прокси мы ушли именно
            # из-за него, а они лишь не смогли подстраховать.
            cooldown = steam_cooldown_remaining("listings")
            if steam_refused:
                head = (
                    "⛔ Оба маршрута закрыты.\n"
                    "1) Steam ответил 429 на ПРЯМОЙ адрес — это первопричина.\n"
                    f"2) Запасной путь через прокси не сработал: {scrub(str(e))}"
                )
                tail = (
                    f"Прямой адрес на кулдауне ещё {cooldown / 60:.0f} мин, "
                    f"следующий прогон пойдёт через прокси."
                    if cooldown > 0
                    else "Следующий прогон снова начнёт с прямого адреса."
                )
            else:
                head = (
                    f"Запрос не пропустил прокси-сервис (не Steam): {scrub(str(e))}"
                )
                tail = (
                    "Steam в кулдаун не отправляю: он в этом прогоне не отказывал. "
                    "Следующий прогон снова начнёт с прямого адреса."
                )
            raise SteamRateLimited(
                f"{head}\n"
                f"Пул: {STEAM_POOL.failure_hint()}.\n"
                f"Отказ прокси — на стороне провайдера: кончился трафик, превышен "
                f"лимит одновременных сессий, истекла сессия или сменились "
                f"логин/пароль. Проверь /proxycheck и личный кабинет.\n"
                f"{tail}"
            )
        ...
        try:
            if resp.status == 429:
                ...
                # Отсюда два разных выхода, и путать их дорого.
                #
                # Кончился ПУЛ — забанены все адреса, какие у нас есть, идти
                # больше некуда, и кулдаун области честен.
                #
                # Кончился БЮДЖЕТ ПОПЫТОК на одном предмете, а свободные адреса
                # остались — совсем другое дело. Бюджет ограничен
                # STEAM_RETRY_HARD_CAP (по умолчанию 10), а адресов в пуле
                # бывает под полсотни. То есть после десяти смен маршрута на
                # ОДНОМ упрямом предмете мы отправляли на кулдаун всю область,
                # имея наготове три десятка рабочих адресов, да ещё и писали в
                # сообщении «свободных прокси нет». Один предмет останавливал
                # прогон на полчаса, а при повторах кулдаун рос до шести часов.
                #
                # Теперь в этом случае падает только предмет: исключение
                # НЕ SteamRateLimited, поэтому прогон не прерывается (см.
                # bot._watchlist_scan_item), и следующий предмет начинает со
                # своим свежим бюджетом.
                #
                # Само по себе это не превращается в долбёжку забаненного
                # Steam: каждый получивший 429 адрес откладывается на
                # LISTINGS_PROXY_COOLDOWN, так что если банят подряд всех,
                # свободные кончатся сами и кулдаун поставится первой веткой.
                free = STEAM_POOL.available()
                if free:
                    note_retry("бюджет попыток")
                    raise RuntimeError(
                        f"Steam ответил 429 на всех маршрутах, отведённых этому предмету "
                        f"(бюджет {listings_retry_budget()}). Свободные адреса ещё есть "
                        f"({free}) — область на кулдаун не отправляю, следующий предмет "
                        f"пойдёт как обычно."
                    )
            ...
            # Тело ответа оборвалось на середине: заголовки пришли, HTTP 200
            # получен, Content-Length пообещал N байт, а соединение закрылось
            # на M < N. В проде 2026-09-13 так падали 12 предметов из 635 —
            # «received 15746 of 49948 bytes» и подобное.
            #
            # Это поломка транспорта, а не отказ Steam: он уже ответил, и
            # ответил успешно. Повторять такое нужно обязательно, и раньше это
            # не работало по чисто структурной причине: соединение ловилось
            # своим except выше, а ЧТЕНИЕ ТЕЛА оказалось за пределами всей
            # логики повторов — исключение улетало мимо неё прямо в общий
            # обработчик предмета, и предмет просто выпадал из прогона.
            #
            # Адрес меняем, но сбойным НЕ помечаем. Обрыв на ротируемом шлюзе —
            # обычное дело: адрес за логином меняется раз в десять минут, и
            # длинный ответ может попасть ровно на смену. Выбивать за это
            # рабочий прокси из пула значит лечить чужую болезнь.
            note_retry("обрыв тела")
            if attempts_left <= 0:
                raise
            attempts_left -= 1
            route = STEAM_POOL.next() or route
            log.warning(
                "fetch_all_listings: ответ по %r оборвался (%s) — повторяю ту же "
                "страницу через %s",
                market_hash_name, e,
                mask_proxy(route) if route else "прямой адрес",
            )
            continue
        finally:

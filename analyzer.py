...
# Предельный разброс между окнами цены Steam, в процентах. Выше — цена
# считается неустойчивой и в арбитраж не идёт (см. find_arbitrage_offers).
# 40% выбрано с запасом: обычные колебания ликвидного предмета за три месяца
# заметно меньше, а случаи, которые нас подвели, расходились в 2-3 раза.
    ...
    *,
    ...
    for l in listings:
        if l.steam_price is None or l.steam_price <= 0:
            ...
            continue
        # Сравниваем суточную цену с недельной. Если они разъезжаются, предмет
        # либо резко двинулся в цене, либо продаж так мало, что среднее скачет —
        # в обоих случаях "скидка" отражает разброс данных, а не выгоду.
        #
        # История двух неверных подходов, чтобы к ним не возвращались. Сначала
        # тут стояло "окно 30/90 дней — выбрасываем": не сработало, потому что у
        # спорных предметов продажи шли каждый день и окно last_24h у них было.
        # Потом разброс считался по ВСЕМ четырём окнам: тоже неверно — предмет
        # мог честно подешеветь за три месяца и быть стабильным сейчас, а
        # проверка выбрасывала его как неустойчивый.
        if (
            l.steam_price_spread_pct is not None
            and l.steam_price_spread_pct > MAX_PRICE_SPREAD_PCT
        ):
            dropped["цена Steam неустойчива"] += 1

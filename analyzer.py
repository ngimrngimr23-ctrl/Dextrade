    ...
    *,
    ...
    for l in listings:
        if l.steam_price is None or l.steam_price <= 0:
            ...
            continue
        # Отсекаем не по возрасту окна, а по согласованности окон между собой.
        #
        # Сначала здесь стояло правило "окно 30/90 дней — выбрасываем", но оно
        # оказалось и неверным, и слишком строгим: у предметов из спорной
        # подборки продажи шли каждый день неделю, то есть окно last_24h у них
        # было, и завышенная цена пришла вовсе не из старого окна. А заодно
        # правило выбрасывало стабильный неликвид, где цена за 30 дней ничем
        # не хуже.
        #
        # Разъезжающиеся окна — признак honest: либо предмет резко изменился в
        # цене, либо продаж так мало, что среднее скачет. В обоих случаях
        # "скидка" отражает разброс данных, а не выгоду.
        if (

...
class ProxyPool:
    ...
    # СНОВА 0 (2026-09-16, вечер). Я поднял этот порог до 8 утром того же дня и
    # ошибся; замер закрыл вопрос окончательно:
    #
    #     /proxycheck: работают 38 из 134, не отвечают 96
    #     Разных адресов: 38 из 38
    #
    # Отказы не общие, а частые: примерно 72% логинов отвечают отказом, 28%
    # работают. При такой доле восемь отказов подряд выпадают сплошь и рядом —
    # 0.72^8 ≈ 7% на каждое окно из восьми попыток, а окон за прогон десятки.
    # То есть порог срабатывал бы постоянно и укладывал бы 38 ЖИВЫХ логинов
    # из-за обычного невезения. Ровно это мне и сказали прямым текстом: «если
    # один даёт сбой, то он бракует все».
    #
    # Тормозом служит не этот счётчик, а две другие вещи: PROXY_ATTEMPTS_PER_PAGE
    # ограничивает перебор восемью попытками на страницу (при 28% живых
    # вероятность промахнуться мимо рабочего — 0.72^8, около 7%), а отказавший
    # логин ложится на минуту и в этот перебор больше не попадает.
    #
    # Порог остаётся настраиваемым: если провайдер однажды действительно
    # закроет дверь целиком, PROXY_GATEWAY_GIVE_UP_AFTER задаётся окружением.
    # Но по умолчанию предполагать отказ двери по серии — значит ошибаться в
    # ту сторону, где теряются рабочие адреса.

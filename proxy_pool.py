...
class ProxyPool:
    ...
    # существует.
    # 0 — не откладывать пул целиком НИКОГДА, только отказавший логин.
    #
    # Умолчание сменилось с 8 на 0 после замера /proxycheck 2026-09-13:
    #
    #     работают 49 из 96 ... Разных адресов: 49 из 49
    #
    # Две вещи разом. Первая: за каждым логином СВОЙ исходящий адрес — пул
    # настоящий, а не иллюзия, и вся идея «за логинами одна дверь, значит
    # отказ общий» неверна в самой посылке. Вторая: отказы не общие, а
    # примерно случайные, около половины на попытку — похоже на лимит
    # одновременных сессий у провайдера, а не на закрытый аккаунт.
    #
    # При случайных отказах любой порог на серию срабатывает сам собой: восемь
    # неудач подряд при вероятности 0.5 выпадают раз на 256 попыток, а за
    # прогон их сотни. То есть порог гарантированно и регулярно укладывал
    # девяносто шесть рабочих логинов из-за случайного совпадения.
    #
    # Тормоз при настоящем отказе провайдера всё равно остаётся, и он честнее:
    # каждый отказавший логин ложится на минуту сам, пул пустеет, и next()
    # возвращает None примерно через 96 быстрых неудач. Это ограничивает
    # издержки без единого предположения о природе отказа.
    # ВЕРНУЛСЯ С 0 НА 8 (2026-09-16), и вот почему — прежнее рассуждение было
    # верным ровно до тех пор, пока «сдаться» означало «прогон умер».
    #
    # Тогда выключить порог было правильно: отложить весь пул значило остаться
    # без единого маршрута, и случайная серия отказов (при вероятности 0.5
    # восемь подряд выпадают раз на 256 попыток, а их за прогон сотни) хоронила
    # девяносто шесть рабочих логинов ни за что.
    #
    # Теперь у запросов есть запасной прямой маршрут (csfloat_client,
    # fetch_listings_page). «Сдаться» больше не значит «умереть» — значит
    # «перестать долбиться в закрытую дверь и пойти другой». А цена перебора
    # измерена: 2026-09-16 шлюз отказывал ВСЕМ, примерно секунда на логин,
    # и /orders не укладывался в свои 45 секунд, потому что каждая из шести
    # страниц честно платила по восемь отказов.
    #
    # Ложное срабатывание при этом стоит 60 секунд работы без прокси, а не
    # сорванного прогона. Это приемлемо; обратное — нет.
    ...
    def note_gateway_refusal(self, proxy: str, seconds: float, reason: str = "") -> bool:
        ...
        if not self.GATEWAY_GIVE_UP_AFTER:
            # Ходу нет только этому логину. Остальные 95 к его отказу
            # отношения не имеют — у каждого свой адрес, проверено /proxycheck.
            return False
    ...
    def failure_hint(self) -> str:
        ...
        if self.single_gateway():
            # Раньше здесь стояло «это не независимые адреса, отказ шлюза общий
            # для всех, перебирать их нечего». Замер /proxycheck 2026-09-13 это
            # опроверг: «работают 49 из 96 ... Разных адресов: 49 из 49» — за
            # каждым логином свой исходящий адрес, и отказ одного о других не
            # говорит ничего. Текст, который советует НЕ перебирать, вреден
            # вдвойне: именно перебор и есть правильное поведение.
            return (
                f"{self.describe()} — вход у логинов общий "
                f"({next(iter(self.hosts()))}), но исходящий адрес у каждого свой "
                f"(проверено /proxycheck). Отказ одного логина о других ничего не "
                f"говорит: чаще всего это лимит одновременных сессий у провайдера"
            )

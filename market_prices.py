...
# бывает — его бы разобрали мгновенно.
#
# А вот ПОЧЕМУ так выходило — вопрос, в котором я дважды ошибся, поэтому здесь
# только то, что проверено, и отдельно то, что предполагается.
#
# Проверено (2026-08-29, живой прогон + /pricecheck):
#   * цена Steam бралась из самого свежего НЕПУСТОГО окна прайс-листа с
#     откатом вплоть до квартального. Из 32077 цен суточных было 17678, то
#     есть 14399 записей несли ценник до 90 дней давности. Подешевевший за
#     квартал скин выглядел огромной скидкой. Это и была главная причина;
#   * предметы за один-два цента, где процент огромен, а прибыль копеечная.
#
# Предполагалось, но НЕ проверено:
#   * что LOOT.FARM и подобные обменники публикуют цену выкупа (сколько дают
#     тебе), а не цену покупки. Это была догадка по симптому, а не факт о
#     содержимом файла: сам файл ни разу не открывали. После починки окна
#     догадка стала ещё слабее — старая цена Steam объясняет те же находки без
#     всяких предположений о LOOT.FARM. Проверяется открытием площадки на
#     любом предмете из выдачи; до тех пор площадку из списка не убираем.

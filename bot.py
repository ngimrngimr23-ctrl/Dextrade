...
# Потолок страниц за прогон; одна страница = 50 лотов = ОДИН запрос.
#
# Десять — это до 500 лотов за прогон. Столько стало можно, потому что лимит
# CSFloat (200 запросов в час) считается ПО АДРЕСУ: с шестью резидентными
# прокси бюджет 1200/час, и автоскан раз в 5 минут тратит от силы 120.
#
# Реально страниц почти всегда качается меньше: при sort_by=highest_discount
# лоты идут по убыванию скидки, и fetch_market останавливается, как только
# хвост страницы ушёл ниже порога отбора (stop_below_discount). То есть глубина
# берётся там, где выгодные лоты есть, а на пустом рынке прогон стоит одну-две
# страницы.
#
# Переопределяется переменной окружения — подобрать глубину под свой тариф
# прокси можно без правки кода.
ARB_PAGES_PER_SCAN = int(os.environ.get("ARB_PAGES_PER_SCAN", "10"))

# Ниже — рассуждение, из-за которого страниц когда-то стало ровно одна. Оно
# верное для одного адреса и объясняет, почему глубина не бесплатна.
#
# Было 4 страницы, и это ошибка сразу по двум причинам.
#
# Во-первых, бюджет. Лимит CSFloat на ключ — 200 запросов в окне. 4 страницы
# каждые ARB_INTERVAL_MINUTES — это 48 запросов в час, 1152 в сутки: четверть
# часового окна тратится вхолостую постоянно, а в суточное мы не влезаем в
# шесть раз.
#
# Во-вторых, страницы за первой тем хуже, чем дальше: сортировка по убыванию
# скидки означает, что глубина докупает всё менее выгодные лоты. Слепо качать
# фиксированное число страниц — значит платить за данные, которые отбор
# выбросит. Отсюда и ранняя остановка вместо простого потолка.

...
async def _run_watchlist_scan(
    ...
    try:
        ...
        # Судьба находок. Счётчики считались давно, но в отчёт не попадали, и
        # последний шаг конвейера оставался невидимым: прогон 21:12 нашёл ОДИН
        # оффер, дедуп его съел, в чат не ушло ничего — и по логу это выглядело
        # ровно как «не нашлось». Три числа закрывают вопрос «почему молчит».
        log.info(
            "watchlist: находки — прошли отбор %d, из них новых %d, отправлено %d",
            stats.offers, stats.fresh, stats.sent,
        )
        if stats.offers and not stats.fresh:
            log.info(
                "watchlist: все %d находок уже присылались раньше — дедуп держит "
                "их %d дней, это не пустой прогон",
                stats.offers, WATCH_OFFER_TTL_SECONDS // 86400,
            )
...
async def _dips_scan_body(send, chat_id: int, saved: dict, *, quiet: bool = False) -> None:
    ...
    for dip in top:
        ...
        #
        # Медиана есть только у priceoverview. Когда он на кулдауне, цена
        # приходит запасным путём из листингов (pricing._lowest_price_from_listings),
        # и там ни медианы, ни объёма — то есть проверить нечем ровно так же,
        # как если бы ответа не было вовсе. Поэтому и ведём себя так же, как в
        # ветке выше: с включённым фильтром молчим, с выключенным показываем
        # оценку. Раньше эта ветка отвергала находку безусловно, и выключенный
        # фильтр не спасал — непоследовательность, из-за которой команда
        # замолкала совсем.
        if quote.median is None:
            if min_week_volume:
                no_median += 1
                continue
            verified.append((dip, quote.ask, dip.today, dip.drop_pct, None, False))
            continue
    ...
    if not verified:
        if not quiet:
            ...
            if no_median:
                # Называем настоящую причину, а не её следствие. «Нет медианы»
                # звучит как особенность данных, хотя на деле это почти всегда
                # кулдаун priceoverview после 429.
                cooldown = steam_cooldown_remaining("pricing")
                if cooldown > 0:
                    why.append(
                        f"Steam не отдаёт цены после 429 (ещё {cooldown / 60:.0f} мин) — "
                        f"без них не с чем сравнивать: {no_median}"
                    )
                else:
                    why.append(
                        f"Steam не вернул медиану сделок, сравнивать не с чем — {no_median}"
                    )
            if unverified:
            ...
            # Подсказка ровно там, где человек упёрся, а не в справочнике.
            if no_median or unverified:
                text += (
                    "\n\nПросадка считается по медиане состоявшихся сделок, а её "
                    "отдаёт только priceoverview. Пока он недоступен, сравнивать "
                    "нижнюю заявку со средней за месяц нельзя — это разные "
                    "величины, и разница в ~13% появилась бы на каждом предмете "
                    "сама собой.\n"
                    "Если готов смотреть неподтверждённые оценки: /dips - - 0 "
                    "(выключит фильтр ликвидности)."
                )
            await send(text)
        return
...
#
# СТО ВОСЕМЬДЕСЯТ, А НЕ СОРОК ПЯТЬ. Прежние 45 секунд я взял не из замера, а
# из испуга: команда тогда молча висела на мёртвом шлюзе, и хотелось любого
# предела. Число вышло заведомо неверным — /arbnow на 1500 лотов честно идёт
# полторы минуты, то есть медленный, но РАБОТАЮЩИЙ скан этот таймаут рубил,
# и /orders списывал на сеть собственное нетерпение.
#
# Причины торопиться с тех пор ушли, каждая отдельной правкой: перебор
# ограничен восемью попытками на страницу, полоса садится на заведомо рабочий
# логин, прайс-лист греется ДО таймера, а при исчерпанной квоте скан вообще
...
async def _order_candidates_from_arb(
    ...
    # Хватит ли ключу бюджета на сам скан.
    #
...
async def orders_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Прайс-лист греем ДО таймера, и это не мелочь.
    #
    # Он лежит в файле внутри контейнера, а контейнер пересоздаётся на каждом
    # деплое — значит первый прогон после деплоя качает его целиком: весь
    # каталог CS2 одним JSON, со своим тридцатисекундным таймаутом, плюс
    # разбор двадцати тысяч записей. На бесплатном тарифе это легко съедает
    # почти все сорок пять секунд, отведённые широкому скану.
    #
    # Раньше эта загрузка шла ВНУТРИ wait_for, и её время списывалось на
    # CSFloat. Сообщение при этом уверенно называло виновным прокси-сервис —
    # из-за чего мы искали поломку в прокси, когда её там не было. Работа эта
    # к тому же общая: просадкам прайс-лист нужен ровно так же, и греть его
    # под чужим таймером неправильно вдвойне.
    warm_started = time.monotonic()
    try:
        await get_csgotrader_price_details()
    except Exception as e:  # noqa: BLE001 — без прайс-листа дальше разберутся сами
        log.warning("orders: прайс-лист не подняли: %s", e)
    ...
    try:
        ...
        # только то, что не успели; кто именно тормозил, видно в логе выше.
        log.info(
            "orders: широкий скан не уложился в %.0f с (прайс-лист до этого "
            "занял %.1f с) — беру только просадки",
            ORDERS_ARB_TIMEOUT, warm_took,
        )
        await update.message.reply_text(
            f"⚠️ Широкий скан не уложился в {ORDERS_ARB_TIMEOUT:.0f} с — беру "
            "кандидатов только из просадок. Чем именно занят, видно в логах."
        )
        candidates = []
    except (CSFloatRateLimited, CSFloatError) as e:

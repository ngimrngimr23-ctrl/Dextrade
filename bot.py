...
# Тоже только в памяти процесса — сбрасываются при рестарте бота на стандартные 5.0/7.0.
_chat_defaults: dict[int, dict] = {}
...
def _get_defaults(chat_id: int) -> tuple[float, float]:
    d = _chat_defaults.get(chat_id)
    if d is None:
...
def _split_args(args: list[str]) -> tuple[str, float | None, float | None]:
    ...
    Название предмета может состоять из нескольких слов ("AK-47 | Сланец"),
...
async def _resolve_market_hash_name(
    ...
    Если это текст (в т.ч. на русском) — ищет предмет по открытой базе
    ByMykel/CSGO-API (не через Steam, так что не подвержено блокировке
    датацентровых IP). При нескольких подходящих вариантах (разный износ
    ...
    if not results:
        await update.message.reply_text(
            f"Ничего не нашёл по «{raw}». Проверь название или пришли ссылку на предмет напрямую."
        )
        return None
...
async def scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    chat_id = update.effective_chat.id
    def_min, def_max = _get_defaults(chat_id)
    if not args:
        await update.message.reply_text(
            f"Формат: /scan <ссылка или название предмета> [мин$ стикеров={def_min:.0f}] [макс наценка%={def_max:.0f}]\n"
            f"Название можно писать и по-русски: /scan AK-47 | Сланец (Field-Tested)"
        )
        return
...
async def setdefaults(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        cur_min, cur_max = _get_defaults(chat_id)
        await update.message.reply_text(
    ...
    _chat_defaults[chat_id] = {"min_value": min_value, "max_markup": max_markup}
    await update.message.reply_text(
...
async def scanfile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text(
            ...
            f"Название можно писать и по-русски: /scanfile AK-47 | Сланец (Field-Tested)"
        )
        return
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _run_analysis(update, session["listings"], session["min_value"], session["max_markup"])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Название можно писать по-русски: /scan AK-47 | Сланец\n\n"

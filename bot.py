...
async def scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text(
            f"Формат: /scan <ссылка на предмет Steam Market> [мин$ стикеров={def_min:.0f}] [макс наценка%={def_max:.0f}]"
        )
        return

    url = args[0]
    min_value = float(args[1]) if len(args) > 1 else def_min
    max_markup = float(args[2]) if len(args) > 2 else def_max
...
async def scanfile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text(
            f"Формат: /scanfile <ссылка на предмет Steam Market> [мин$ стикеров={def_min:.0f}] [макс наценка%={def_max:.0f}]"
        )
        return

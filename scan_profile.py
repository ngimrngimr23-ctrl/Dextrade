...
class ScanProfile:
    ...
    def render(
        ...
        # Тег целиком внутри одной строки: отчёт режется на куски ПО СТРОКАМ
        # (_chunk_lines), и тег, начатый в одной строке и закрытый в другой,
        # при разрезе между ними превратился бы в незакрытый — Telegram такое
        # сообщение не принимает вовсе.
        lines.append(
            f"<b>Фазы</b> <i>(суммы по всем предметам, идут внахлёст — "
            f"поэтому больше {wall:.1f} с реального времени)</i>"
        )

...
# Числа, из-за которых одна и та же поломка рассыпается на десяток «разных»
# причин. Ровно этот случай 2026-09-13: двенадцать оборванных ответов Steam
# приехали в чат как двенадцать отдельных причин по одной штуке, потому что в
# текст входит «(received 15746 of 49948 bytes)» — байты у каждого свои. Из-за
# лимита в три причины отчёт показывал три случайных из двенадцати и приписку
# «и ещё 9 причин», хотя причина была ОДНА.
#
# Порог в четыре знака выбран так, чтобы уцелело осмысленное (коды 400 и 429,
# «5 из 7»), а счётчики байт, миллисекунды и идентификаторы схлопнулись.
